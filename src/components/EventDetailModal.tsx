                    ...
                    {renderEvent ? renderEvent(event) : (
                        <div>
                            ...
                            <div className="mt-6 space-y-3">
                                {event.details && (
                                    <div className="rounded-xl border border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800/40 p-3.5">
                                        <h3 className="text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider mb-2.5">Reference Detail</h3>
                                ...
                                {event.customer && (
                                    <div className="rounded-xl border border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800/40 p-3.5">
                                        <h3 className="text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider mb-2.5">Customer Profile</h3>
                                        <div className="flex items-center gap-3">
                                            {/* Avatar */}
                                            <div className="w-9 h-9 rounded-full bg-indigo-100 dark:bg-indigo-900/40 flex items-center justify-center shrink-0">
                                                <span className="text-sm font-bold text-indigo-600 dark:text-indigo-400">
                                                    {event.customer.name.charAt(0).toUpperCase()}
                                                </span>
                                            </div>
                                            <div className="min-w-0">
                                                ...
                                                {event.customer.company && (
                                                    <div className="text-xs text-zinc-500 dark:text-zinc-400 truncate">{event.customer.company}</div>
                                                )}
                                            </div>
                                        </div>
                                        {event.customer.contact && (
                                            <div className="mt-2.5 pt-2.5 border-t border-zinc-200 dark:border-zinc-700/60 flex items-center gap-2">
                                                ...
                                                <span className="text-xs text-zinc-600 dark:text-zinc-300">{event.customer.contact}</span>
                                            </div>
                                        )}
                                    </div>
                                )}
                                ...
                                {event.asset && (
                                    <div className="rounded-xl border border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800/40 p-3.5">
                                        <h3 className="text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider mb-2.5">Assigned Unit</h3>
                                        <div className="flex items-center gap-3">
                                            {/* Unit icon */}
                                            ...
                                            <div className="min-w-0 flex-1">
                                                ...
                                                {event.asset.id && (
                                                    <div className="text-xs text-zinc-500 dark:text-zinc-400">ID: {event.asset.id}</div>
                                                )}
                                                {event.asset.contract && (
                                                    <div className="text-xs text-zinc-500 dark:text-zinc-400">Reference: {event.asset.contract}</div>
                                                )}
                                            </div>
                                            {event.asset.licensePlate && (
                                                <span className="shrink-0 text-[11px] font-bold tracking-widest px-2 py-1 rounded-md bg-zinc-200 dark:bg-zinc-700 text-zinc-700 dark:text-zinc-200 border border-zinc-300 dark:border-zinc-600 font-mono">
                                                    {event.asset.licensePlate}
                                                </span>
                                            )}
                                        </div>
